	...
	while (gf_bs_available(mov->movieFileMap->bs)) {
		*bytesMissing = 0;
		...
		case GF_ISOM_BOX_TYPE_MOOV:
			if (mov->moov) {
// ROBT - In the file where I saw this, things worked when I ignored the second MOOV.
				GF_LOG(GF_LOG_ERROR, GF_LOG_CONTAINER, ("[iso file] Duplicate MOOV detected! And Ignored\n"));
//				return GF_ISOM_INVALID_FILE;
                break;
			}
